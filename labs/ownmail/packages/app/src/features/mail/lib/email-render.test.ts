...
describe('shadowStyleText', () => {
	...
	it('uses zero-specificity defaults plus trusted readable-mode constraints', () => {
		...
		expect(css).not.toContain(':where(html, body, table, img, video, svg, canvas){max-width:100%!important;}')
		expect(css).toContain(':where(table){min-width:0!important;table-layout:auto;}')
		expect(css).not.toContain('[style*="white-space" i][style*="nowrap" i]')
		expect(css).toContain(':where(img, video, svg, canvas){height:auto;}')
