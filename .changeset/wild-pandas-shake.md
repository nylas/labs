...
`ownmail eject` carries those bindings into the project it hands you, reading them from the template's own build output so `eject` and `deploy` cannot drift apart.
